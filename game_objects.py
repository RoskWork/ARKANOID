    ...
    def update(self, paddle, launch_ball=False):
        ...
        if self.is_slowed:
            ...
            if self.slow_timer <= 0:
                # Restore original speed (double current speed as it was halved)
                self.speed_x = (self.speed_x / abs(self.speed_x)) * self.base_speed
                self.speed_y = (self.speed_y / abs(self.speed_y)) * self.base_speed
                self.is_slowed = False
        ...
        if self.rect.colliderect(paddle.rect) and self.speed_y > 0:
            if paddle.has_glue:
                ...
                relative_intersection_x = self.rect.centerx - paddle.rect.centerx
                normalized_intersection_x = relative_intersection_x / (paddle.rect.width / 2)
                self.speed_x = normalized_intersection_x * self.base_speed
...
class PowerUp:
    PROPERTIES = {
        ...
        'multi_ball': {'color': (100, 100, 255), 'char': 'M', 'message': 'MULTI-BALL!'},
        'life_up': {'color': (0, 255, 0), 'char': '+', 'message': 'EXTRA LIFE!'},
    }
